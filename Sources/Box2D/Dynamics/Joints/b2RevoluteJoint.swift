  ...
  override func initVelocityConstraints(_ data: inout b2SolverData) {
    ...
    if m_enableLimit && fixedRotation == false {
      let jointAngle = aB - aA - m_referenceAngle
        if abs(m_upperAngle - m_lowerAngle) < 2.0 * b2Settings.Instance.b2_angularSlop {
    ...
    if m_enableLimit && m_limitState != b2LimitState.inactiveLimit && fixedRotation == false {
      ...
      if m_limitState == b2LimitState.equalLimits {
        // Prevent large angular corrections
          let C = b2Clamp(angle - m_lowerAngle, -b2Settings.Instance.b2_maxAngularCorrection, b2Settings.Instance.b2_maxAngularCorrection)
        ...
        // Prevent large angular corrections and allow some slop.
          C = b2Clamp(C + b2Settings.Instance.b2_angularSlop, -b2Settings.Instance.b2_maxAngularCorrection, 0.0)
        limitImpulse = -m_motorMass * C
      }
      else if m_limitState == b2LimitState.atUpperLimit {
        ...
        // Prevent large angular corrections and allow some slop.
          C = b2Clamp(C - b2Settings.Instance.b2_angularSlop, 0.0, b2Settings.Instance.b2_maxAngularCorrection)
        limitImpulse = -m_motorMass * C
      }
