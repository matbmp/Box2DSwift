...
open class b2DistanceJoint : b2Joint {
  ...
  override func initVelocityConstraints(_ data: inout b2SolverData) {
    ...
    let length = m_u.length()
      if length > b2Settings.Instance.b2_linearSlop {
      m_u *= 1.0 / length
    }
    else {
      m_u.set(0.0, 0.0)
  ...
  override func solvePositionConstraints(_ data: inout b2SolverData) -> Bool {
    ...
    var C = length - m_length
      C = b2Clamp(C, -b2Settings.Instance.b2_maxLinearCorrection, b2Settings.Instance.b2_maxLinearCorrection)
    ...
    data.positions[m_indexB].a = aB
      ...
      return abs(C) < b2Settings.Instance.b2_linearSlop
  }
