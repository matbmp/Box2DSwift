...
//      let color = b2Color(0.3, 0.9, 0.9)
  ...
  */
  ...
  func solveTOI(_ step: b2TimeStep) {
    if m_TOIIsland == nil {
        m_TOIIsland = b2Island(2 * b2Settings.Instance.b2_maxTOIContacts, b2Settings.Instance.b2_maxTOIContacts, 0, m_contactManager.m_contactListener)
    }
    else {
        m_TOIIsland.reset(2 * b2Settings.Instance.b2_maxTOIContacts, b2Settings.Instance.b2_maxTOIContacts, 0, m_contactManager.m_contactListener)
    }
    ...
    while true {
      ...
      //for (var c = m_contactManager.m_contactList; c != nil; c = c!.m_next) {
        ...
        // Prevent excessive sub-stepping.
          if c!.m_toiCount > b2Settings.Instance.b2_maxSubSteps {
          c = c!.m_next
          continue
        }
        ...
        if minContact == nil || 1.0 - 10.0 * b2Settings.Instance.b2_epsilon < minAlpha {
  ...
  func drawShape(_ fixture: b2Fixture, _ xf: b2Transform, _ color: b2Color) {
    ...
    case .polygon:
      ...
      let vertexCount = poly.m_count
        assert(vertexCount <= b2Settings.Instance.b2_maxPolygonVertices)
      var vertices = [b2Vec2]()
