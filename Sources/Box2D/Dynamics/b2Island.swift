...
open class b2Island {
  ...
  func solve(_ profile: inout b2Profile, _ step: b2TimeStep, _ gravity: b2Vec2, _ allowSleep: Bool) {
    ...
    for i in 0 ..< m_bodyCount {
      ...
      let translation = h * v
        if b2Dot(translation, translation) > b2Settings.Instance.b2_maxTranslationSquared {
            let ratio = b2Settings.Instance.b2_maxTranslation / translation.length()
        v *= ratio
      }
      
      let rotation = h * w
        if rotation * rotation > b2Settings.Instance.b2_maxRotationSquared {
            let ratio = b2Settings.Instance.b2_maxRotation / abs(rotation)
        w *= ratio
      }
    ...
    if allowSleep {
        var minSleepTime = b2Settings.Instance.b2_maxFloat
      
        let linTolSqr = b2Settings.Instance.b2_linearSleepTolerance * b2Settings.Instance.b2_linearSleepTolerance
        let angTolSqr = b2Settings.Instance.b2_angularSleepTolerance * b2Settings.Instance.b2_angularSleepTolerance
      ...
      for i in 0 ..< m_bodyCount {
        ...
        if minSleepTime >= b2Settings.Instance.b2_timeToSleep && positionSolved {
        for i in 0 ..< m_bodyCount {
          let b = m_bodies[i]
          b.setAwake(false)
        }
