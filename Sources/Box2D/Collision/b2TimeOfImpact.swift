...
public func b2TimeOfImpact(_ output: inout b2TOIOutput, input: b2TOIInput) {
  ...
  let totalRadius = proxyA.m_radius + proxyB.m_radius
    let target = max(b2Settings.Instance.b2_linearSlop, totalRadius - 3.0 * b2Settings.Instance.b2_linearSlop)
    let tolerance = 0.25 * b2Settings.Instance.b2_linearSlop
  ...
  while true {
    ...
    while true {
      ...
      pushBackIter += 1
        ...
        if pushBackIter == b2Settings.Instance.b2_maxPolygonVertices {
        break
      }
    }
