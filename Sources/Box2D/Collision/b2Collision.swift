...
open class b2WorldManifold : CustomStringConvertible {
  ...
  */
    open var points = [b2Vec2](repeating: b2Vec2(0, 0), count: b2Settings.Instance.b2_maxManifoldPoints)
  /**
  a negative value indicates overlap, in meters
  */
    open var separations = [Float](repeating: 0, count: b2Settings.Instance.b2_maxManifoldPoints)
  /**
  ...
  open func initialize(manifold: b2Manifold,
    ...
    case .circles:
      ...
      let pointB = b2Mul(xfB, manifold.points[0].localPoint)
        if b2DistanceSquared(pointA, pointB) > b2Settings.Instance.b2_epsilon * b2Settings.Instance.b2_epsilon {
...
public struct b2AABB : CustomStringConvertible {
  ...
  */
  ...
  public func rayCast(_ input: b2RayCastInput) -> b2RayCastOutput? {
      var tmin = b2Settings.Instance.b2_minFloat
      var tmax = b2Settings.Instance.b2_maxFloat
    ...
    for i in 0 ..< 2 {
        if absD[i] < b2Settings.Instance.b2_epsilon {
        // Parallel.
...
public func b2TestOverlap(shapeA: b2Shape, indexA: Int,
  ...
  b2Distance(&output, cache: &cache, input: input)
    return output.distance < 10.0 * b2Settings.Instance.b2_epsilon
}
