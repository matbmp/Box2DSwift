...
struct b2TempPolygon {
    var vertices = [b2Vec2](repeating: b2Vec2(), count: b2Settings.Instance.b2_maxPolygonVertices)
    var normals = [b2Vec2](repeating: b2Vec2(), count: b2Settings.Instance.b2_maxPolygonVertices)
  var count = 0
}
...
class b2EPCollider {
  ...
  func Collide(_ edgeA: b2EdgeShape, _ xfA: b2Transform, _ polygonB: b2PolygonShape, _ xfB: b2Transform) -> b2Manifold {
    ...
    for i in 0 ..< polygonB.m_count {
      ...
      m_radius = 2.0 * b2Settings.Instance.b2_polygonRadius
    ...
    let clipPoints1 = b2ClipSegmentToLine(inputVertices: ie, normal: rf.sideNormal1, offset: rf.sideOffset1, vertexIndexA: rf.i1)
      ...
      if clipPoints1.count < b2Settings.Instance.b2_maxManifoldPoints {
      return manifold
    }
    ...
    let clipPoints2 = b2ClipSegmentToLine(inputVertices: clipPoints1, normal: rf.sideNormal2, offset: rf.sideOffset2, vertexIndexA: rf.i2)
      ...
      if clipPoints2.count < b2Settings.Instance.b2_maxManifoldPoints {
      return manifold
    }
    ...
    var pointCount = 0
      for i in 0 ..< b2Settings.Instance.b2_maxManifoldPoints {
  ...
  func ComputePolygonSeparation() -> b2EPAxis {
    ...
    for i in 0 ..< m_polygonB.count {
      ...
      // Adjacency
      if b2Dot(n, perp) >= 0.0 {
          if b2Dot(n - m_upperLimit, m_normal) < -b2Settings.Instance.b2_angularSlop {
          continue
        }
      }
      else {
          if b2Dot(n - m_lowerLimit, m_normal) < -b2Settings.Instance.b2_angularSlop {
          continue
        }
      }
