    ...
    _ proxyB: b2DistanceProxy, _ transformB: b2Transform)
  {
    ...
    if m_count > 1 {
      ...
      let metric2 = getMetric()
        if metric2 < 0.5 * metric1 || 2.0 * metric1 < metric2 || metric2 < b2Settings.Instance.b2_epsilon {
        // Reset the simplex.
        m_count = 0
      }
    }
...
public func b2Distance(_ output: inout b2DistanceOutput, cache: inout b2SimplexCache, input: b2DistanceInput) {
  ...
  var saveCount = 0
    ...
    var distanceSqr1 = b2Settings.Instance.b2_maxFloat
  ...
  while iter < k_maxIters {
    ...
    // Ensure the search direction is numerically fit.
      if d.lengthSquared() < b2Settings.Instance.b2_epsilon * b2Settings.Instance.b2_epsilon {
...
//    var wBLocal: b2Vec2
  ...
  if input.useRadii {
    ...
    let rB = proxyB.m_radius
      ...
      if output.distance > rA + rB && output.distance > b2Settings.Instance.b2_epsilon {
